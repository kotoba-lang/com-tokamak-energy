(ns tokamak_energy.main
  "Kotodama WASM entrypoint for the Tokamak_energy clean-room actor (L4) — Clojure port.
  ...
  facts (`tokamak_energy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "tokamak_energy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"tokamak_energy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tokamak_energy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :tokamak_energy-compat/ready)
